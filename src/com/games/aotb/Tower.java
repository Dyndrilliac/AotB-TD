...
public class Tower extends Sprite
{
	...
	private float  cdMod   = 1.0f;       // Modifies the cooldown for this tower by a percentage. Allows you to upgrade basic towers.
	private float  dmgMod  = 1.0f;       // Modifies the damage for this tower. Allows you to upgrade basic towers.
	...
	private float   totalCooldown    = (Tower.COOLDOWN * this.getCooldownModifier());
			...
			if (!towerNotAllowed)
			{
				for (int i = 0; i < TowerMain.enemyClone.length; i++)
				{
					...
					else
					{
						...
						{
							tempPaths[Enemy.getEnemyArray().size() + i] = new Path(TowerMain.enemyClone[i], TowerMain.currentLevel.getGoalPoint()[0], TowerMain.tmxLayer, TowerMain.currentLevel);
	...
	public boolean fire(final Enemy target, final Tower source, final Scene scene, final ArrayList<Enemy> enemyArray, final BaseGameActivity myContext)
	{
		...
		if ((elapsed > (long)totalCooldown) && (!moveable))
		{
			this.bulletSprite = new Projectile(source.getMidX(), source.getMidY(), 10f, 10f, bulletTexture, getVertexBufferObjectManager(), scene);
	...
	public static void loadSound(final SoundManager soundmanager, final SimpleBaseGameActivity activity)
	{
		try
		{
			firing_sound = SoundFactory.createSoundFromAsset(soundmanager, activity, Tower.FIRING_SOUND);
		}
	...
	public void setHitAreaShown(final Scene scene, final boolean showHitArea)
	{
		if (moveable)
		{
			this.towerRangeGood.setPosition(this.getWidth() / 2 - this.towerRangeGood.getWidth() / 2, this.getHeight() / 2 - this.towerRangeGood.getHeight() / 2);
			this.towerRangeBad.setPosition(this.getWidth() / 2 - this.towerRangeBad.getWidth() / 2, this.getHeight() / 2 - this.towerRangeBad.getHeight() / 2);
		}
		else
		{
			this.towerRangeGood.setPosition(this.getX() + this.getWidth() / 2 - this.towerRangeGood.getWidth() / 2, this.getY() + this.getHeight() / 2 - this.towerRangeGood.getHeight() / 2);
			this.towerRangeBad.setPosition(this.getX() + this.getWidth() / 2 - this.towerRangeBad.getWidth() / 2, this.getY() + this.getHeight() / 2 - this.towerRangeBad.getHeight() / 2);
		}
				 ...
				 final VertexBufferObjectManager vbom)
	{
		...
		this.towerRangeGood = new TowerRange(0, 0, this.hitAreaTextureGood, getVertexBufferObjectManager());
		this.towerRangeBad = new TowerRange(0, 0, this.hitAreaTextureBad, getVertexBufferObjectManager());
		this.towerRangeGood.setPosition(this.getWidth() / 2 - this.towerRangeGood.getWidth() / 2, this.getHeight() / 2 - this.towerRangeGood.getHeight() / 2);
		this. towerRangeBad.setPosition(this.getWidth() / 2 - this.towerRangeBad.getWidth() / 2, this.getHeight() / 2 - this.towerRangeBad.getHeight() / 2);

...
public class Enemy extends Sprite implements Cloneable
{
	...
	public Enemy clone()
	{
		final Enemy returnEnemy =
			new Enemy(this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.getTextureRegion(), this.getVertexBufferObjectManager());
	...
	public int getCurrentLink()
	{
		...
		if (this.getPath().getXYPath().size() > 2)
		{
			...
			{
				if (((this.getPath().getXYPath().get(i - 1).getY_float() <= this.getY()) && (this.getY() <= this.getPath().getXYPath().get(i).getY_float()))
					|| ((this.getPath().getXYPath().get(i).getY_float() <= this.getY()) && (this.getY() <= this.getPath().getXYPath().get(i - 1).getY_float())))
				{
					if (((this.getPath().getXYPath().get(i - 1).getX_float() <= this.getX()) && (this.getX() <= this.getPath().getXYPath().get(i).getX_float()))
						|| ((this.getPath().getXYPath().get(i).getX_float() <= this.getX()) && (this.getX() <= this
							.getPath()
						.getXYPath()
						.get(i - 1)
						.getX_float())))
					{
						...
						{
							double m =
								(this.getPath().getXYPath().get(i).getY_float() - this.getPath().getXYPath().get(i - 1).getY_float())
									/ (this.getPath().getXYPath().get(i).getX_float() - this.getPath().getXYPath().get(i - 1).getX_float());
							// b = y-mx
	...
	public void startMoving(final BaseGameActivity myContext)
	{
		if (this.getPath() != null)
		{
			...
			if (tempPath != null)
			{
				...
				this.registerEntityModifier(this.getTrajectory());
			}
		}
	}
	...
	public void stop()
	{
		this.unregisterEntityModifier(this.getTrajectory());
	}

...
public class BuildTowerTouchHandler implements IOnAreaTouchListener
{
								 ...
								 final float touchAreaLocalX, final float touchAreaLocalY)
	{
		...
		else if (sceneTouchEvent.isActionMove())
		{
			if (BuildTowerTouchHandler.getTower() == null)
			{
				...
				BuildTowerTouchHandler.tower = new Tower(bulletTexture, newX, newY, Tower.WIDTH, Tower.HEIGHT, towerTexture, 
														 hitAreaTextureGood, hitAreaTextureBad, scene, towerArray, 
	...
	private boolean towerTouchEvent(final TouchEvent sceneTouchEvent, final Tower thisTower)
	{
		...
		{
			this.distTraveled += Math.sqrt((Math.pow(this.lastX - sceneTouchEvent.getX(), 2)) + (Math.pow(this.lastY - sceneTouchEvent.getY(), 2))) * TowerMain.zoomCamera.getZoomFactor();
			this.lastX = sceneTouchEvent.getX();
			this.lastY = sceneTouchEvent.getY();
		...
		else if (sceneTouchEvent.isActionUp())
		{

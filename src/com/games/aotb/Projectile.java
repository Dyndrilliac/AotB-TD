...
public class Projectile extends Sprite
{
	...
	public Projectile(final float x, final float y, final float width, final float height, final TextureRegion textureRegion, final VertexBufferObjectManager vbom, final Scene scene)
	{
	...
	public void setTarget(final Tower tower, final Enemy enemy)
	{
		this.target = enemy;
		this.source = tower;
		this.totalDamage = (Tower.DAMAGE * source.getDamageModifier());
	}
	...
	public void shoot(final ArrayList<Enemy> enemyArray, final BaseGameActivity myContext)
	{
		...
		double totalDist = Math.sqrt(Math.pow(this.target.getPath().getXYPath().get(origLink).getX_float() - this.target.getX(), 2) + Math.pow(this.target.getPath().getXYPath().get(origLink).getY_float() - this.target.getY(), 2));
		TrajectoryReturn trajReturn = findTrajectory(origLink, -1);
		...
		while ((trajReturn.t > (totalDist / Enemy.SPEED)) && (curLink<this.target.getPath().getXYPath().size()-1))
		{
			...
			totalDist += Math.sqrt(Math.pow(this.target.getPath().getXYPath().get(curLink).getX_float() - this.target.getPath().getXYPath().get(curLink-1).getX_float(), 2) + Math.pow(this.target.getPath().getXYPath().get(curLink).getY_float() - this.target.getPath().getXYPath().get(curLink-1).getY_float(), 2));
		}
		...
		this.trajectory.addModifierListener(new IModifierListener<IEntity>()
		{
			...
			public void onModifierFinished(final IModifier<IEntity> modifier, final IEntity item)
			{
				...
				// Enemy takes damage.
				target.setInboundDamage(target.getInboundDamage() - totalDamage);
				
				if (target.takeDamage(totalDamage, source.getDamageType()) < 1)
				{
					// Enemy dies.
					if (target.isAlive())
					{
					...
					enemyArray.remove(target); 
				}
			}
		});
		
		registerEntityModifier(this.trajectory);
		this.target.setInboundDamage(this.target.getInboundDamage() + totalDamage);
	}
	...
	protected TrajectoryReturn findTrajectory(final int curLink, final double prevDist)
	{
		...
		trajectoryReturn.enemyAngle = Math.atan2(this.target.getPath().getXYPath().get(curLink).getY_float() - this.target.getPath().getXYPath().get(curLink-1).getY_float(), this.target.getPath().getXYPath().get(curLink).getX_float() - this.target.getPath().getXYPath().get(curLink-1).getX_float());
		...
		else
		{
			...
			trajectoryReturn.x = this.target.getPath().getXYPath().get(curLink-1).getX_float() + Math.cos(backAngle)*prevDist + this.target.getMidX() - this.target.getX();
			trajectoryReturn.y = this.target.getPath().getXYPath().get(curLink-1).getY_float() + Math.sin(backAngle)*prevDist + this.target.getMidY() - this.target.getY();
		}

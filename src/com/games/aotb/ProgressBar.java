...
public class ProgressBar extends Rectangle
{
		...
		final VertexBufferObjectManager vbom)
	{
		...
		// Draw lines.
		for (Line frameLine: this.frameLines)
		{
			this.attachChild(frameLine);
		}
	}

	public ProgressBar clone(final Enemy returnEnemy)
	{
		final ProgressBar returnProgressBar =
			new ProgressBar(this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.maxValue, this.currentValue, this.getVertexBufferObjectManager());
	...
	public ProgressBar setFrameColor(final float red, final float green, final float blue, final float alpha)
	{
		for (Line frameLine: this.frameLines)
		{
			frameLine.setColor(red, green, blue, alpha);
		}

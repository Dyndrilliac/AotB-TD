...
public class Path implements Cloneable
{
	...
	{
		@Override
		public float getCost(final IPathFinderMap<Enemy> pathFinderMap, final int fromX, final int fromY, final int toX, final int toY, final Enemy entity)
		{
			return 1.0f;
		}
	};
	...
	protected IPathFinderMap<Enemy> pathMap = new IPathFinderMap<Enemy>()
	{
		...
		public boolean isBlocked(final int x, final int y, final Enemy entity)
		{
			try
			{
				TMXTile tmxTile = TowerMain.tmxLayer.getTMXTileAt(TowerMain.getXFromCol(x), TowerMain.getYFromRow(y));
				TMXProperties<TMXTileProperty> tmxTileProperties = TowerMain.tmxTiledMap.getTMXTileProperties(tmxTile.getGlobalTileID());
			...
			{
				for (int i = 0; i < level.getGoalPoint().length; i++)
				{
					if ((x == level.getGoalPoint()[i].getX_int()) && (y == level.getGoalPoint()[i].getY_int()))
					{
						return false;
					}
				}
				
				for (int i = 0; i < level.getStartPoint().length; i++)
				{
					if ((x == level.getStartPoint()[i].getX_int()) && (y == level.getStartPoint()[i].getY_int()))
					{
						return false;
					}
				}
	...
	public boolean checkRemainingPath(final int x, final int y)
	{
		if (this.rcPath != null)
		{
			...
			for (int i = (length - 1); i >= 0; i--)
			{
				...
				else if (xArray[i] == enemyCol && yArray[i] == enemyRow)
				{
					return false;
				}
			}
	...
	public double getCurrentAngle()
	{
		...
		if (this.getXYPath().size() > 1)
		{
			...
			{
				if (((this.getXYPath().get(i).getY_float() <= this.enemy.getY()) && (this.enemy.getY() <= this.getXYPath().get(i + 1).getY_float())) || ((this.getXYPath().get(i + 1).getY_float() <= this.enemy.getY()) && (this.enemy.getY() <= this.getXYPath().get(i).getY_float())))
				{
					if (((this.getXYPath().get(i).getX_float() <= this.enemy.getX()) && (this.enemy.getX() <= this.getXYPath().get(i + 1).getX_float())) || ((this.getXYPath().get(i + 1).getX_float() <= this.enemy.getX()) && (this.enemy.getX() <= this.getXYPath().get(i).getX_float())))
					{
						...
						{
							m = (this.getXYPath().get(i + 1).getY_float() - this.getXYPath().get(i).getY_float()) / (this.getXYPath().get(i + 1).getX_float() - this.getXYPath().get(i).getX_float());
	...
	public Path(final Enemy enemy, final Point end, final TMXLayer tmxlayer, final Level level, final ArrayList<Point> xyPath, final org.andengine.util.algorithm.path.Path rcPath)
	{
		this.enemy = enemy;
		this.waypoints = new ArrayList<Point>();
		this.iterator = this.waypoints.iterator();
